...
base_sellout <- readRDS("base_historica_sellout.rds") %>% 
  mutate(
...
# Usar siempre Date para cálculos
fecha_base_date <- as.Date(max(base_sellout$Fecha, na.rm = TRUE))

# Versión texto solo para mostrar en el UI
fecha_base <- format(fecha_base_date, "%d/%m/%Y")
...
# Carga de datos
# sellout_semanal <- readRDS("sellout_semanal.rds") %>% 
#   rename(Fecha_Carga = `Fecha Carga`) %>% 
#   mutate(
#     SKU = as.character(SKU),
#     Codigo_Femaco = as.character(Codigo_Femaco),
#     Nombre_Producto = as.character(Nombre_Producto),
#     Producto = paste0(SKU, "-", Codigo_Femaco, "-", Nombre_Producto)
#   ) %>% 
#   dplyr::select(Fecha_Carga, Producto, Local, Local_ID, `Stock Físico`)
...
stock_sku_local <- sellout_semanal

# stock_sku_local <- sellout_semanal %>% 
#   group_by(Producto, Local, Local_ID) %>%
#   summarise(Stock = sum(`Stock Físico`, na.rm = TRUE), .groups = "drop")

# base_sellout <- base_sellout %>% 
#   left_join(stock_sku_local, by = c("Producto","Local"))
...
ui <- fluidPage(
  ...
  div(class = "custom-header",
      ...
      div(class = "custom-title", "Sell Out por Producto y Local")
  ),
  tags$p(style = "margin-left: 15px; font-style: italic;",
         paste("Datos actualizados al", fecha_base)),
  sidebarLayout(
    sidebarPanel(
      id = "sidebar",
      ...
      conditionalPanel(
        ...
        div(style = "margin-top: 10px; margin-bottom: 10px;",
            dateRangeInput(
              ...
              start = as.Date(min(base_sellout$Fecha, na.rm = TRUE)),
              end = as.Date(max(base_sellout$Fecha, na.rm = TRUE)),
              min = as.Date(min(base_sellout$Fecha, na.rm = TRUE)),
              max = as.Date(max(base_sellout$Fecha, na.rm = TRUE)),
              format = "dd-MM-yyyy",
...
server <- function(input, output, session) {
  ...
  filtrar_datos <- function(df) {
    if (input$modo_fecha == "rango_absoluto") {
      ...
      df <- df %>%
        filter(format(Fecha, "%m-%d") >= inicio,
               format(Fecha, "%m-%d") <= fin)
    }
    if (input$subcategoria != "Todas") df <- df %>% filter(Subcategoria == input$subcategoria)
    # if (input$grupo != "Todos") df <- df %>% filter(Grupo == input$grupo)
    if (input$conjunto != "Todos") df <- df %>% filter(Conjunto == input$conjunto)
  ...
  data_filtrada <- reactive({
    filtrar_datos(base_sellout)
  })
  
  data_filtrada_stock <- reactive({
    filtrar_datos2(stock_sku_local)
  })
  ...
  excel_btn <- list(
    extend = "excel",
    filename = "reporte_sellout",   # nombre descarga
    exportOptions = list(
  ...
  output$tabla_sku <- renderDT({
    ...
    resumen <- if (input$modo == "Ventas") {
      agregar_tabla(df, nivel = "Producto", variable = "Venta_Neta")
    } else {
      agregar_tabla(df, nivel = "Producto", variable = "Cantidad")
    }
    
    # después de construir 'resumen' (con nivel = "Producto")
    resumen <- resumen %>%
      left_join(df %>% distinct(Producto, SKU), by = "Producto") %>%
      relocate(SKU, .before = Producto)
    
    total_stock <- data_filtrada_stock() %>% 
      dplyr::select(Producto, Local, Stock) %>% 
      unique() %>% 
      group_by(Producto) %>% 
      summarise(Stock = sum(Stock, na.rm = TRUE), .groups = "drop")
    
    resumen <- resumen %>% 
      left_join(total_stock, by = c("Producto"))
    ...
    coldefs <- c(render_rules, list(list(visible = FALSE, targets = 0)))  # 0 = columna SKU
  ...
  output$tabla_local <- renderDT({
    ...
    resumen <- if (input$modo == "Ventas") {
      agregar_tabla(df, nivel = "Local", variable = "Venta_Neta")
    } else {
      agregar_tabla(df, nivel = "Local", variable = "Cantidad")
    }
    ...
    resumen <- resumen %>% 
      left_join(total_stock, by = c("Local"))
  ...
  output$tabla_categoria <- renderDT({
    ...
    resumen <- if (input$modo == "Ventas") {
      agregar_tabla(df, nivel = "Categoria", variable = "Venta_Neta")
    } else {
      agregar_tabla(df, nivel = "Categoria", variable = "Cantidad")
    }
  ...
  output$tabla_subcategoria <- renderDT({
    ...
    resumen <- if (input$modo == "Ventas") {
      agregar_tabla(df, nivel = "Subcategoria", variable = "Venta_Neta")
    } else {
      agregar_tabla(df, nivel = "Subcategoria", variable = "Cantidad")
    }
  ...
  output$tabla_total <- renderDT({
    ...
    resumen <- if (input$modo == "Ventas") {
      agregar_tabla(df, nivel = "", variable = "Venta_Neta")
    } else {
      agregar_tabla(df, nivel = "", variable = "Cantidad")
    }
  ...
  observeEvent(input$local, {
    ...
    if ("Todss" %in% input$zona && length(input$zona) > 1) {
  ...
  observe({
    updateSelectInput(session, "subcategoria", choices = c("Todas", sort(unique(base_sellout$Subcategoria))))
    # updateSelectInput(session, "grupo", choices = c("Todos", sort(unique(base_sellout$Grupo))))
    updateSelectInput(session, "categoria", choices = c("Todas", sort(unique(base_sellout$Categoria))))
    updateSelectInput(session, "conjunto", choices = c("Todos", sort(unique(base_sellout$Conjunto))))
    updateSelectInput(session, "estado", choices = c("Todos", sort(unique(base_sellout$Estado))))
    # updateSelectInput(session, "zona", choices = c("Todas", sort(unique(base_sellout$Zona))))
    updateSelectInput(session, "zona", choices = c("Todas", sort(unique(base_sellout$Zona))), selected = "Todas")
    updateSelectInput(session, "local", choices = c("Todos", sort(unique(base_sellout$Local))))
    updateSelectInput(session, "producto_id", choices = c("Todos", obtener_opciones_producto(base_sellout)))
    updateSelectInput(session, "formato", choices = c("Todos", sort(unique(base_sellout$Formato))))
  })
